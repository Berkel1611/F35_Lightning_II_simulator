...
public class Plane : MonoBehaviour, IDamageable
{
    ...
    public float PredictAngleOfAttack { get; private set; }
    ...
    void CalculateAngleOfAttack()
    {
        ...
        AngleOfAttack = Utilities.CalculateAlpha(LocalVelocity);
        AngleOfAttackYaw = Utilities.CalculateBeta(LocalVelocity);
    }
    ...
    void UpdateAirData()
    {
        ...
        float speedFeet = speed * metersToFeet; // ft/s
    ...
    void CalculateState(float dt)
    {
        ...
        LocalVelocity = invRotation * Velocity; // transform velocity to local space
        LocalAngularVelocity = invRotation * Rigidbody.angularVelocity; // transform angular velocity to local space
    ...
    void UpdateControls(float dt)
    {
        ...
        float predictedAlpha = state.alpha;
        PredictAngleOfAttack = Utilities.ConvertAngle360To180(predictedAlpha);
    ...
    void UpdateAerodynamics(float alpha, float beta)
    {
        ...
        AerodynamicState currentState = new AerodynamicState
        {
            ...
            velocity = Utilities.ConvertVectorToAerospace(LocalVelocity) * metersToFeet,
            angularVelocity = Utilities.ConvertAngleToAerospace(LocalAngularVelocity),
            airData = airData,
            alpha = alpha,
            beta = beta,
        ...
        // aeroForces in pounds, convert to Newtons
        var forces = Utilities.ConvertVectorToUnity(aeroForces) * poundsForceToNewtons;
        Rigidbody.AddRelativeForce(forces);

        // aeroAngularAcceleration changes angular velocity directly
        Vector3 avCorrection = Utilities.ConvertAngleToUnity(aeroAngularAcceleration);
